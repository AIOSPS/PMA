...
public class NumberToArabicWord {
    ...
    public String convertNumberToWord(String total) {
        ...
        if (total != null && !total.equals("")) {
            ...
            if (total.indexOf(".") >= 0) {
                ...
                dec=Integer.parseInt(total.substring(total.indexOf(".") + 1, total.length()));
                String decStr=dec+"";
                if(dec>0)
                {
                 if(decStr.length()<3)
    ...
    private int toNumber(String num1, String num2) {
        return new Integer(num1).intValue() + new Integer(num2).intValue();
    }
    ...
    public static String convertFloatFormat(Double val, int n) {

        double newValue = Math.round(val.doubleValue() * Math.pow(10, n)) / Math.pow(10, n);
        String str = (newValue == Math.floor(new Double(newValue).intValue())) ? new Double(newValue).intValue() + ".00" : ((newValue * 10 == Math.floor(newValue * 10)) ? newValue + "0" : newValue + "");
        return str;
    }
